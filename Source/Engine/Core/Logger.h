...
#include "Framework/Singleton.h" // poor practice but neccesary for our purposes
...
namespace kiko 
{
	...
	{
	public:
		Logger(LogLevel logLevel = LogLevel::Info, std::ostream* ostream = &std::cout, const std::string& filename = "log.txt") : // psuedo default constructor 
			m_logLevel { logLevel },
			m_ostream{ ostream }
		{
	...
	//extern Logger g_logger; // no longer neccesary due to changes made 
