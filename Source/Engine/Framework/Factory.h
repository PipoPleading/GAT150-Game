...
namespace kiko
{
	...
	public:
		std::unique_ptr<class Object> Create() override
		{
			...
			return std::make_unique<T>(); //illegal return types are occuring here as of 8/16
		}
	};
	...
	public:
		...
		template<typename T>
		void RegisterPrototype(const std::string& key, std::unique_ptr<T> prototype); //hell
	...
	{
		INFO_LOG("Class prototype registered: " << key);

		m_registry[key] = std::make_unique<PrototypeCreator<T>>(std::move(prototype)); //probably missing a pointer here???
