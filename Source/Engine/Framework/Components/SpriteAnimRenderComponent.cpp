#include "Framework/Components/SpriteAnimRenderComponent.h" //misspelled something, will need to copy everythin to correct directories
#include "Renderer/Renderer.h"
#include <Framework/Resource/ResourceManager.h>
//need to include renderer, actor, and something else
...
namespace kiko
{
	...
	void kiko::SpriteAnimRenderComponent::Update(float dt)
	{
		frameTimer -= dt;
		if (frameTimer <= 0)
		{
	...
	void SpriteAnimRenderComponent::UpdateSource()
	{
		...
		int column = (frame - 1) % m_sequence->numColumns; //need to review notation, but idea is present 
	...
	//void kiko::SpriteAnimRenderComponent::Draw(Renderer& renderer) //need to cross reference 
	//{

	//}

	void kiko::SpriteAnimRenderComponent::Read(const json_t& value)
	{
		SpriteComponent::Read(value); 

		// read in animation sequences
		auto temp = HAS_DATA(value, sequences);
